...
using System.Linq;
using TransformEnhancer.Runtime;
using UnityEditor;
...
namespace TransformEnhancer.Editor
{
    ...
    public partial class ImitatedRectTransformEditor : UnityEditor.Editor
    {
        // Consts
        ...
        ConstrainProportionsTransformScaleReflection m_ConstrainProportionsScale;
        ...
        public virtual void OnEnable()
        {
            ...
            {
                m_ConstrainProportionsScale = new ConstrainProportionsTransformScaleReflection(m_LocalScale.vector3Value);
            }
            ...
            ManipulationToolUtilityReflection.AddHandleDragChange(this, ImitatedRectTransformEditorReflection.HandleDragChangeMethod);
        ...
        public virtual void OnDisable()
        {
            ...
            ManipulationToolUtilityReflection.SubHandleDragChange(this, ImitatedRectTransformEditorReflection.HandleDragChangeMethod);
            SceneView.duringSceneGui -= DrawAnchorsOnSceneView;
        ...
        void HandleDragChange(string handleName, bool dragging)
        {
            ...
            switch (handleName)
            {
                case RectToolReflection.ChangingLeft: animBool = m_ChangingLeft; break;
                case RectToolReflection.ChangingRight: animBool = m_ChangingRight; break;
                case RectToolReflection.ChangingPosY: animBool = m_ChangingPosY; break;
                case RectToolReflection.ChangingWidth: animBool = m_ChangingWidth; break;
                case RectToolReflection.ChangingBottom: animBool = m_ChangingBottom; break;
                case RectToolReflection.ChangingTop: animBool = m_ChangingTop; break;
                case RectToolReflection.ChangingPosX: animBool = m_ChangingPosX; break;
                case RectToolReflection.ChangingHeight: animBool = m_ChangingHeight; break;
                case RectToolReflection.ChangingPivot: animBool = m_ChangingPivot; break;
        ...
        void DrawAnchorsOnSceneView(SceneView sceneView)
        {
            ...
            if (!sceneView.drawGizmos || !EditorGUIUtilityReflection.IsGizmosAllowedForObject(target))
                return;

            if (SceneViewReflection.ActiveEditors.Contains(this) || SceneViewReflection.ActiveEditors.Contains(target))
                return;
        ...
        void SetFadingBasedOnMouseDownUp(ref AnimBool animBool, Event eventBefore)
        {
        ...
        void SetFadingBasedOnControlID(ref AnimBool animBool, int id)
        {
            var focusedView = (EditorWindow.focusedWindow == null ? null : EditorWindow.focusedWindow.GetParent());
            if (GUIUtility.keyboardControl == id && GUIViewReflection.Current == focusedView)
            {
                animBool.value = true;
                m_KeyboardControlIDs[id] = animBool;
            }
            else if ((GUIUtility.keyboardControl != id || GUIViewReflection.Current != focusedView) && m_KeyboardControlIDs.ContainsKey(id))
            {
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                if (gui.GetDrivenByObject() != null)
                {
                    anyDriven = true;
                    if ((gui.GetDrivenProperties() & (DrivenTransformProperties.AnchoredPositionX | DrivenTransformProperties.SizeDeltaX)) != 0)
                        anyDrivenXPositionOrSize = true;
                    if ((gui.GetDrivenProperties() & (DrivenTransformProperties.AnchoredPositionY | DrivenTransformProperties.SizeDeltaY)) != 0)
            ...
            {
                if (targets.Length == 1)
                    EditorGUILayout.HelpBox("Some values driven by " + (target as RectTransform).GetDrivenByObject().GetType().Name + ".", MessageType.None);
                else
            ...
            DrawPositionAndSize(anyWithoutParent, anyDrivenXPositionOrSize, anyDrivenYPositionOrSize);
            ...
            // Scale
            s_ScaleDisabledMask[0] = targets.Any(x => ((x as RectTransform).GetDrivenProperties() & DrivenTransformProperties.ScaleX) != 0);
            s_ScaleDisabledMask[1] = targets.Any(x => ((x as RectTransform).GetDrivenProperties() & DrivenTransformProperties.ScaleY) != 0);
            s_ScaleDisabledMask[2] = targets.Any(x => ((x as RectTransform).GetDrivenProperties() & DrivenTransformProperties.ScaleZ) != 0);
        ...
        static void Vector3FieldWithDisabledMash(Rect position, SerializedProperty property, GUIContent label, bool[] disabledMask)
        {
            ...
            position = EditorGUIReflection.MultiFieldPrefixLabel(position, id, label, 3);
            position.height = EditorGUIUtility.singleLineHeight;
            SerializedProperty cur = property.Copy();
            cur.NextVisible(true);
            EditorGUIReflection.MultiPropertyFieldInternal(position, s_XYZLabels, cur, PropertyVisibilityReflection.OnlyVisible, disabledMask);

            EditorGUI.EndProperty();
        }

        private LayoutDropdownWindowReflection m_DropdownWindow;
        ...
        void LayoutDropdownButton(bool anyWithoutParent)
        {
            ...
            using (new EditorGUI.DisabledScope(anyWithoutParent))
            {
                if (EditorGUI.DropdownButton(dropdownPosition, GUIContent.none, FocusType.Passive, "label"))
                {
                    ...
                    m_DropdownWindow = new LayoutDropdownWindowReflection(serializedObject);
                    PopupWindow.Show(dropdownPosition, m_DropdownWindow.PopupWindow);
                }
            }

            if (!anyWithoutParent)
            {
                LayoutDropdownWindowReflection.DrawLayoutMode(new RectOffset(7, 7, 7, 7).Remove(dropdownPosition), m_AnchorMin, m_AnchorMax, m_AnchoredPosition, m_SizeDelta);
                LayoutDropdownWindowReflection.DrawLayoutModeHeadersOutsideRect(dropdownPosition, m_AnchorMin, m_AnchorMax, m_AnchoredPosition, m_SizeDelta);
            }
        }
        ...
        void SmartPositionAndSizeFields(bool anyWithoutParent, bool anyDrivenX, bool anyDrivenY)
        {
            ...
            if (anyNonStretchX || anyWithoutParent || anyDrivenX)
            {
                ...
                SetFadingBasedOnControlID(ref m_ChangingPosX, EditorGUIUtilityReflection.LastControlID);
                EditorGUI.EndProperty();
            }
            else
            {
                ...
                SetFadingBasedOnControlID(ref m_ChangingLeft, EditorGUIUtilityReflection.LastControlID);
                EditorGUI.EndProperty();
                EditorGUI.EndProperty();
            }
            ...
            if (anyNonStretchY || anyWithoutParent || anyDrivenY)
            {
                ...
                SetFadingBasedOnControlID(ref m_ChangingPosY, EditorGUIUtilityReflection.LastControlID);
                EditorGUI.EndProperty();
            }
            else
            {
                ...
                SetFadingBasedOnControlID(ref m_ChangingTop, EditorGUIUtilityReflection.LastControlID);
                EditorGUI.EndProperty();
                EditorGUI.EndProperty();
            }
            ...
            if (anyNonStretchX || anyWithoutParent || anyDrivenX)
            {
                ...
                SetFadingBasedOnControlID(ref m_ChangingWidth, EditorGUIUtilityReflection.LastControlID);
                EditorGUI.EndProperty();
            }
            else
            {
                ...
                SetFadingBasedOnControlID(ref m_ChangingRight, EditorGUIUtilityReflection.LastControlID);
                EditorGUI.EndProperty();
                EditorGUI.EndProperty();
            }
            ...
            if (anyNonStretchY || anyWithoutParent || anyDrivenY)
            {
                ...
                SetFadingBasedOnControlID(ref m_ChangingHeight, EditorGUIUtilityReflection.LastControlID);
                EditorGUI.EndProperty();
            }
            else
            {
                ...
                SetFadingBasedOnControlID(ref m_ChangingBottom, EditorGUIUtilityReflection.LastControlID);
                EditorGUI.EndProperty();
                EditorGUI.EndProperty();
            }
        ...
        void SmartAnchorFields()
        {
            ...
            EditorGUILayout.Space(EditorGUIReflection.VerticalSpacingMulti);

            anchorRect.y += EditorGUIUtility.singleLineHeight + EditorGUIReflection.VerticalSpacingMulti;
            Vector2Field(anchorRect,
        ...
        void BlueprintButton(Rect position)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                ToolsReflection.RepaintAllToolViews();
            }
        }
        ...
        {
            using (new EditorGUI.DisabledScope(targets.Any(x => ((x as RectTransform).GetDrivenProperties() & driven) != 0)))
            {
                ...
                float newValue = EditorGUIReflection.DoFloatField(EditorGUIReflection.DelayedTextEditor, positionField, positionLabel, id, value, EditorGUIReflection.FloatFieldFormatString, EditorStyles.textField, true);
                if (EditorGUI.EndChangeCheck())
                {
                          ...
                          SerializedProperty vec2Property, GUIContent label)
        {
            ...
            EditorGUIUtility.labelWidth = EditorGUIReflection.CalcPrefixLabelWidth(s_XYLabels[0]);
        ...
        void DrawRect(Rect rect, Transform space, bool dotted)
        {
            ...
            {
                RectHandlesReflection.DrawDottedLineWithShadow(kShadowColor, kShadowOffset, p0, p1, kDottedLineSize);
                RectHandlesReflection.DrawDottedLineWithShadow(kShadowColor, kShadowOffset, p1, p2, kDottedLineSize);
                RectHandlesReflection.DrawDottedLineWithShadow(kShadowColor, kShadowOffset, p2, p3, kDottedLineSize);
                RectHandlesReflection.DrawDottedLineWithShadow(kShadowColor, kShadowOffset, p3, p0, kDottedLineSize);
            }
        }
        ...
        void OnSceneGUI()
        {
            ...
            if (m_TargetCount == 1)
            {
                RectTransformSnappingReflection.OnGUI();
                ...
                RectTransformSnappingReflection.DrawGuides();
        ...
        void ParentRectPreviewDragHandles(RectTransform gui, Transform space)
        {
            ...
            for (int xHandle = 0; xHandle <= 2; xHandle++)
            {
                for (int yHandle = 0; yHandle <= 2; yHandle++)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    Vector3 newPos = RectHandlesReflection.SideSlider(id, curPos, sideDir, slideDir, size, null, Vector2.zero, -3);
                    if (EditorGUI.EndChangeCheck())
                    {
        ...
        void UpdateTemporaryRect()
        {
            ...
            if (EditorGUIUtility.hotControl == s_ParentDragId)
            {
                ...
                PlayModeViewReflection.RepaintAll();
                return;
            }
        ...
        void AnchorSceneGUI(RectTransform gui, RectTransform guiParent, Transform parentSpace, bool interactive, int minmaxX, int minmaxY, int id)
        {
            ...
            if (evtCopy.type == EventType.MouseDown && GUIUtility.hotControl == id)
            {
                ...
                RectTransformSnappingReflection.CalculateAnchorSnapValues(parentSpace, gui.transform, gui, minmaxX, minmaxY);
            }
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                for (int axis = 0; axis <= 1; axis++)
                {
                    ...
                    float snapSize = HandleUtility.GetHandleSize(newPos) * RectTransformSnappingReflection.SnapThreshold / guiParent.rect.size[axis];
                    snapSize *= parentSpace.InverseTransformVector(axis == 0 ? Vector3.right : Vector3.up)[axis];
                    snappedValue = RectTransformSnappingReflection.SnapToGuides(snappedValue, snapSize, axis);
                    ...
                    if (gui.GetDrivenByObject() != null)
                        RectTransformReflection.SendReapplyDrivenProperties(gui);
                }
            }
        ...
        void DrawPositionDistances(Transform userSpace, Rect rectInParentSpace, Transform parentSpace, RectTransform gui, RectTransform guiParent, float size, int axis, int side, float alpha)
        {
            ...
            RectHandlesReflection.DrawDottedLineWithShadow(kShadowColor, kShadowOffset, posA, posB, kDottedLineSize);
        ...
        void DrawAnchorDistances(Transform parentSpace, RectTransform gui, RectTransform guiParent, float size, float alpha)
        {
            ...
            for (int axis = 0; axis < 2; axis++)
            {
                ...
                if (startValue > 0)
                    DrawLabelBetweenPoints(points[axis, 0] - dir, points[axis, 1] - dir, GUIContentReflection.Temp(startValue + "%"));
                if (midValue > 0)
                    DrawLabelBetweenPoints(points[axis, 1] - dir, points[axis, 2] - dir, GUIContentReflection.Temp(midValue + "%"));
                if (endValue > 0)
                    DrawLabelBetweenPoints(points[axis, 2] - dir, points[axis, 3] - dir, GUIContentReflection.Temp(endValue + "%"));
            }
        }
        ...
        void DrawAnchorRect(Transform parentSpace, RectTransform gui, RectTransform guiParent, int axis, float alpha)
        {
            ...
            for (int side = 0; side < 2; side++)
            {
                ...
                RectHandlesReflection.DrawDottedLineWithShadow(kShadowColor, kShadowOffset, points[side, 0], points[side, 1], kDottedLineSize);
            }
        }
        ...
        void DrawAnchor(Vector3 pos, Vector3 right, Vector3 up)
        {
            ...
            up *= 1.4f;
            right *= 1.4f;
            RectHandlesReflection.DrawPolyLineWithShadow(kShadowColor, kShadowOffset,
                                                         pos,
                                                         pos + up + right * 0.5f,
                                                         pos + right + up * 0.5f,
                                                         pos);
    ...
    public partial class ImitatedRectTransformEditor
    {
        // Position And Size
        protected virtual void DrawPositionAndSize(bool anyWithoutParent, bool anyDrivenX, bool anyDrivenY)
        {
            OnPrePositionAndSizeGUI(anyWithoutParent, anyDrivenX, anyDrivenY);
            OnPositionAndSizeGUI(anyWithoutParent, anyDrivenX, anyDrivenY);
            OnPostPositionAndSizeGUI(anyWithoutParent, anyDrivenX, anyDrivenY);
        }

        protected virtual void OnPrePositionAndSizeGUI(bool anyWithoutParent, bool anyDrivenX, bool anyDrivenY)
        {
        }

        protected virtual void OnPositionAndSizeGUI(bool anyWithoutParent, bool anyDrivenX, bool anyDrivenY)
        {
            SmartPositionAndSizeFields(anyWithoutParent, anyDrivenX, anyDrivenY);
        }

        protected virtual void OnPostPositionAndSizeGUI(bool anyWithoutParent, bool anyDrivenX, bool anyDrivenY)
        {
        }

        // Anchor
        protected virtual void DrawAnchor()
        {
            OnPreAnchorGUI();
            OnAnchorGUI();
            OnPostAnchorGUI();
        ...
        protected virtual void OnPreAnchorGUI()
        {
        }

        protected virtual void OnAnchorGUI()
        {
            SmartAnchorFields();
        }

        protected virtual void OnPostAnchorGUI()
        {
        }

        // Pivot
        protected virtual void DrawPivot()
        {
            OnPrePivotGUI();
            OnPivotGUI();
            OnPostPivotGUI();
        ...
        protected virtual void OnPrePivotGUI()
        {
        }

        protected virtual void OnPivotGUI()
        {
            SmartPivotField();
        }

        protected virtual void OnPostPivotGUI()
        {
        }
        ...
        {
            m_RotationGUI.RotationField(targets.Any(x => ((x as RectTransform).GetDrivenProperties() & DrivenTransformProperties.Rotation) != 0));
        }
        ...
        protected virtual void OnScaleGUI()
        {
            ...
            if (t != null)
            {
                if (m_ConstrainProportionsScale != null
                    && m_ConstrainProportionsScale.Initialize(serializedObject.targetObjects)
                    && !s_ScaleDisabledMask.All(x => x))
                {
                    //AxisModified values [-1;2] : [none, x, y, z]
                    int axisModified = -1;
                    var mixedFields = ConstrainProportionsTransformScaleReflection.GetMixedValueFields(m_LocalScale);
                    ...
                    var mixedFieldsAfterGUI = ConstrainProportionsTransformScaleReflection.GetMixedValueFields(m_LocalScale);
                    ...
                    if (scale != m_LocalScale.vector3Value || mixedFields != mixedFieldsAfterGUI)
                    {
                        if (serializedObject.GetTargetObjectsCount() > 1)
                        {
                            ...
                            if (axisModified != -1)
                            {
                                isScaleDirty = ConstrainProportionsTransformScaleReflection.HandleMultiSelectionScaleChanges(
